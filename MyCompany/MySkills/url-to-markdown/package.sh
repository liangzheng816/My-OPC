#!/usr/bin/env sh
# Package the url-to-markdown skill into a portable zip archive.
#
# Usage:
...
cp "$SCRIPT_DIR/scripts/extract.ts"     "$SKILL_STAGING/scripts/extract.ts"
cp "$SCRIPT_DIR/scripts/fetch-path.ts"  "$SKILL_STAGING/scripts/fetch-path.ts"
cp "$SCRIPT_DIR/scripts/chrome-path.ts" "$SKILL_STAGING/scripts/chrome-path.ts"
cp "$SCRIPT_DIR/scripts/cdp.ts"         "$SKILL_STAGING/scripts/cdp.ts"
