...
Drives: agent-orchestrated `<url> --mode sections` -> NN-<slug>.md, one per section
...
You are an expert technical note-taker writing self-contained chapter notes from an article excerpt.
...
## Details
<2-4 short paragraphs (or sub-headings if the material has clear sub-topics) expanding on the key points. Quote specific phrases from the article where useful.>
...
Rules:
1) Accuracy - only use information present in the provided article excerpt for this section.
